...
- Terraform Surge: crossing into `메가 콤보` on the 8th distinct quick paint grants one bounded energy/stability reward per stroke, logs the beat, and emits a tool-colored layered ring with radial sparks at the last painted cell. `getTerraformSurge()` and `data-terraform-surge-*` expose the transient 16-second signal plus persistent count/cell/tool evidence.
